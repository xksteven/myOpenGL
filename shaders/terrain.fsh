...
void main()
{
	...
	if(fScale >= 0.0 && fScale <= fRange1) vTexColor = texture2D(myTextureSampler, vTexCoord);//vTexColor = texture2D(gSampler[0], vTexCoord);
	else if(fScale <= fRange2)
	{
		...
		vTexColor += texture2D(myTextureSampler, vTexCoord)*fScale;
		vTexColor += texture2D(myTextureSampler, vTexCoord)*fScale2;
	}
	else if(fScale <= fRange3)vTexColor = texture2D(myTextureSampler, vTexCoord); //vTexColor = texture2D(gSampler[1], vTexCoord);
	...
	else vTexColor = texture2D(myTextureSampler, vTexCoord); //vTexColor = texture2D(gSampler[2], vTexCoord);
	...
	vec4 vFinalTexColor = fScale*vTexColor;
	vec4 vMixedColor = vFinalTexColor*vertColor;
	vec4 vDirLightColor = vec4(1,1,1,1);
